...
const getVotesPerEpoch = async (startEpoch: number, endEpoch: number, escrow: string) => {
  ...
  for (let i = startEpoch; i <= endEpoch; i++) {
    ...
    const sortedVotes = filteredVotes.sort((a, b) => b.votingPower - a.votingPower);
...
const writeCSV = (fileName: string, votesPerEpoch: any[]) => {
  ...
  const csv = votesPerEpoch.map(epoch => {
    return epoch.votes.map(vote => {
...
// BEDROCK
const bedrockResult = await getVotesPerEpoch(currentEpoch - 5, currentEpoch, '0xcee845c5db2d8595C6086401a79501c10bb97a0f');
